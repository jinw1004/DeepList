...
resDir = 'E:\wjin\Re-ID-My-Exp-2015-5-15\DeepEmbed_ReId-2015-7-14\result_Exp_VIPeR_09_23\';
mkdir(resDir);

%=========image sampling layer========
% for example, each batch we sample 20 image pairs(x, x+)
imdb = importdata(fullfile(matdir, 'imdb_viper_Top.mat'));
...
for trial = 1:10
    ...
    load(fullfile(matdir, 'preTrain_top_1_1200.mat'));
...
%                                         - lr * ly.momentsLearningRate * res(l).dzdw{3} ; %这个算出来实际上没什么用处
    ...
    %     [loss_trn, cmc_trn] = cnn_test_topHeavy(imdb, id_train, net, batchsize, opts.useGpu, 'auc');
        ...
        title('error');
        drawnow;
        savefig(hh, fullfile(resDir, ['viper_top_' num2str(trial) '.fig']));
        if mod(epoch, 20) == 0 && epoch >= 200
            save(fullfile(resDir, ['viper_top_' num2str(trial) '_' num2str(epoch) '.mat']), 'net', 'cmc_tst', 'info');
        end
        pause(4);
    end
end
