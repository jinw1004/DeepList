...
imdb = importdata(fullfile(matdir, 'imdb_viper_half.mat'));
...
for trial = 2:10
    ...
    load(fullfile(matdir, 'preTrain_half_1_1300.mat'));
...
%                                         - lr * ly.momentsLearningRate * res(l).dzdw{3} ; %这个算出来实际上没什么用处
    ...
    %     [loss_trn, cmc_trn] = cnn_test_topHeavy(imdb, id_train, net, batchsize, opts.useGpu, 'auc');
        ...
        title('error');
        drawnow;
        savefig(hh, fullfile(resDir, ['viper_half_' num2str(trial) '.fig']));
        if mod(epoch, 20) == 0 && epoch >= 200
            save(fullfile(resDir, ['viper_half_' num2str(trial) '_' num2str(epoch) '.mat']), 'net', 'cmc_tst', 'info');
        end
%         pause(4);
    end
end
