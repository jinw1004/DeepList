clear;clc;
addpath(genpath('E:\wjin\DeepList_v1.0'));

matdir = 'E:\wjin\DeepList_v1.0\mat';
resDir = 'E:\wjin\DeepList_v1.0\result_Exp_VIPeR\';
mkdir(resDir);

%=========image sampling layer========
% for example, each batch we sample 20 image pairs(x, x+)
imdb = importdata(fullfile(matdir, 'imdb_viper_200_100_single.mat'));
...
g = 0.5;
opts.useGpu = true ;
% opts.learningRate = [1e-4*ones(1, 100), 5e-5*ones(1, 100), 2e-5*ones(1, 100)];
opts.learningRate = [1e-4*ones(1, 150), 5e-5*ones(1, 100), 2e-5*ones(1, 80), 1e-5*ones(1, 70)];
opts.continue = false ;
...
for trial = 1
    ...
    load(fullfile(matdir, 'preTrain_full_Market_CUHK03D.mat'));
%     load(fullfile(resDir, 'viper_full_BN_1_200_start.mat'));
    ...
    augopts.translation.border = [20 20];
    augopts.imsize = [200 100];
    ...
    %                 g_k = 0.5;
            ...
            dzdfp = dzdfp_list{1}/2000; %k=1,A=3000;k=2,A=6000
%             dzdfp = (dzdfp_list{1} + 0.25*dzdfp_list{2})/2000;
            ...
            res = jk_cnn_singlePart(net, res, dzdy, 'mode', 'train');
...
%                                         - lr * ly.momentsLearningRate * res(l).dzdw{3} ; %这个算出来实际上没什么用处
    ...
    %     [loss_trn, cmc_trn] = cnn_test_topHeavy(imdb, id_train, net, batchsize, opts.useGpu, 'auc');
        ...
        title('error');
        drawnow;
        savefig(hh, fullfile(resDir, ['viper_full_BN_' num2str(trial) '.fig']));
        if mod(epoch, 20) == 0
            save(fullfile(resDir, ['viper_full_BN_' num2str(trial) '_' num2str(epoch) '.mat']), 'net', 'cmc_tst', 'info');
        end
        pause(1);
    end
end
