...
if ispc % Windows  
    ...
    Host_Compiler_Location = '-ccbin "D:\Program Files (x86)\Microsoft Visual Studio 11.0\VC\bin"';  
    CUDA_INC_Location = ['"' getenv('CUDA_PATH')  '\include"'];  
    CUDA_SAMPLES_Location =['"' getenv('NVCUDASAMPLES6_0_ROOT')  '\common\inc"'];  
    PIC_Option = '';  
...
nvccCommandLine = [ ...  
'nvcc -arch compute_20 -code sm_20 --compile ' Host_Compiler_Location ' ' ...  
'-o '  filename '.obj ' ...  
...
mexCommandLine = ['mex ' filename '.obj'  ' -L' CUDA_LIB_Location  ' -lcudart'];  
